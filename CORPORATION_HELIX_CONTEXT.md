...
**Current milestone:** M4.3B --- Corporate entity → domain candidate
evidence\
**Immediate benchmark:** company → unique related legal entities →
evidence-backed candidate domains → approved ASM roots\
...
This malformed sequence has caused repeated confusion during
development, so treat it as a documented upstream-data-quality issue
rather than assuming an `ijson`, SQLite, Unicode, buffering, or
...
The failure was deterministic across repeated reads of the same Golden
Copy, and raw-byte inspection confirmed that the extra period was
physically present in the downloaded JSON.

Operational rule for future GLEIF snapshots:

1.  If a full Golden Copy fails parsing, first check whether the failure
    is the same known `},.` defect before redesigning the importer.
2.  Keep the repair intentionally narrow and logged; do not create a
    permissive "fix arbitrary JSON" layer.
3.  Record the snapshot filename/date and repair count in index
    metadata.
4.  Periodically test newer GLEIF Golden Copies without the repair.
    GLEIF may correct the upstream generator/data record in later
    snapshots.
5.  Do **not** remove the defensive repair merely because one newer
    snapshot is clean. Retire it only after repeated clean snapshots, or
    keep it as a narrowly-scoped compatibility guard for historical
    files.

Expected long-term outcome: this defect will likely disappear from newer
upstream snapshots once the malformed source record or generation path
is corrected. Helix should still remain defensive because historical
Golden Copies may continue to contain it.
...
Corporation Helix should begin producing useful operator output before
the research architecture is "finished."

A likely near-term real-world use case is configuring customer ASM
instances. That workflow requires discovering corporate names,
subsidiaries, former identities, acquisitions, and related legal
entities that should be investigated as potential ASM seeds.
Historically this has required manual Google searches for
mergers/acquisitions and hand-built company-name lists.

Helix can now begin replacing part of that manual workflow.

This is not a customer-facing product milestone yet. The first goal is
an **internal operator tool that saves analyst time and creates real
validation data for Helix.**
...
Initial output should organize candidate identities by why they were
returned, rather than flattening every name into the word "alias."
...
The operator should be able to distinguish a true alias/former name from
a separate related legal entity even though both may be useful ASM
investigation seeds.

## 26.3 Level 2 relationship index --- M4.2B

Do not scan the approximately 1.1 GB RR Golden Copy for every company
query.

Build a persistent local relationship index, likely SQLite, supporting
...
Indexes should at minimum support fast lookup by `child_lei` and
`parent_lei`.
...
The first internal CLI may simply display candidates and ask the
operator to select the intended root:
...
Do not silently choose a similarly named entity when structured evidence
is ambiguous.

## 26.5 Missing Level 1 endpoint policy --- EXPLICIT DECISION

A valid RR relationship must not disappear merely because `lookup_lei()`
cannot enrich one endpoint from the local Level 1 index.

Chosen behavior:

> Preserve the RR relationship and the unresolved endpoint using its
> bare LEI, mark enrichment incomplete, and force conservative
> REVIEW/UNKNOWN behavior. Never invent the missing legal name or
> silently discard the relationship.
...
> Missing Level 1 enrichment does not invalidate an otherwise valid
> GLEIF RR assertion. It limits what Helix knows about an endpoint.

## 26.6 Source layering

The first operator CLI should work from GLEIF without requiring EDGAR
success.
...
EDGAR should add useful coverage where available rather than becoming a
hard dependency for every company.
...
If repeated customer onboarding shows that manual M&A research finds
many valuable historical identities absent from structured providers,
that becomes evidence to resume/invest further in the parked
legal-event/generalization work.
...
The operator CLI is now the practical target that drives M4.2/M4.3
rather than waiting until every research milestone is complete.

## 26.9 First-version scope discipline

For the first internal version:

**Build:** - company-name lookup; - root selection when ambiguous; -
GLEIF Level 1 + supported Level 2 expansion; - canonical relationship
labels; - provenance; - REVIEW/UNKNOWN behavior; - readable terminal
output; - JSON/CSV export if inexpensive.

**Do not block on:** - web UI; - customer-facing polish; -
authentication/permissions; - perfect alias/brand taxonomy; - every
GLEIF relationship class; - every EDGAR filing pattern; - automatic
infrastructure attribution.

Success criterion:

> The CLI saves meaningful time during a real ASM customer onboarding
> and clearly shows the operator what Helix found, why it found it, and
> what still requires manual review.

------------------------------------------------------------------------

# 27. SESSION UPDATE --- 2026-09-04

## 27.1 Current resume point

The GLEIF operator path is now operational enough to move to domain
attribution. Do **not** reopen broad legal-parser research or spend time
polishing the CLI before building the next layer.

Current workflow:

``` text
company / explicit LEI
    -> Level 1 identity lookup
    -> Level 2 relationship lookup
    -> related LEIs
    -> Level 1 enrichment
    -> unique related legal entities
    -> NEXT: candidate domains + infrastructure evidence
    -> AUTO / REVIEW / REJECT
    -> approved registrable domains
...
A HIGH-confidence GLEIF corporate relationship does not prove that a
domain or other infrastructure belongs in ASM scope.

## 27.2 Local generated indexes and two-computer note

The SQLite indexes are generated local artifacts, not repository
source-of-truth. They may exist on one development computer and not
another even when Git code is synchronized. Each machine must have
usable local indexes (or they must later be made reproducibly available
by another mechanism).

On the Mac mini, both indexes were rebuilt successfully on 2026-09-04.

Level 1:

``` text
source: ../Gleif_Golden_Copies/20260804-0800-gleif-goldencopy-lei2-golden-copy.json
scanned: 3,393,123
indexed: 3,393,123
skipped: 0
known narrow syntax repairs: 1
repair: b'},.' -> b'},' near byte 4,666,189,837
DB: data/processed/gleif_lei.sqlite
```

Level 2:

``` text
source: ../Gleif_Golden_Copies/20260817-0800-gleif-goldencopy-rr-golden-copy.json.zip
scanned: 484,558
supported: 258,887
unsupported: 225,671
incomplete: 0
DB: data/processed/gleif_rr.sqlite
```

The Level 1 and Level 2 source dates differ (2026-08-04 vs 2026-08-17).
This is acceptable for current operator testing, but a future
reproducibility cleanup should align snapshots across machines.

Important diagnostic lesson: an empty/truncated SQLite file can
masquerade as a normal lookup miss. A future `helix_doctor.py` should
verify file existence, required tables/indexes, non-zero row counts,
build metadata, and known fixture lookups before operator commands run.

## 27.3 M4.2 --- automatic Level 1 enrichment --- COMPLETE

`code/providers/gleif_lei_index.py` provides runtime Level 1 lookup and
enrichment state handling.

Expected enrichment states:
...
Missing Level 1 enrichment does **not** invalidate a valid RR assertion.
Preserve the RR relationship and bare LEI endpoint, mark enrichment
incomplete, and force conservative REVIEW/UNKNOWN behavior rather than
silently dropping the edge.

`code/canonicalize_gleif_rr.py` now supports `--lei-index`,
automatically enriches child and parent LEIs, retains manual
name/jurisdiction overrides for backward compatibility, and records
Level 1 enrichment metadata.

Validated Desjardins relationship:
...
## 27.4 M4.2B --- persistent Level 2 relationship index --- COMPLETE

`code/build_gleif_rr_index.py` builds a persistent SQLite index for
supported accounting relationships in both lookup directions.
...
Temporal handling was corrected and validated. Do not regress to
treating the first GLEIF `RelationshipPeriod` as the relationship
lifetime. Preserve these period classes separately:

``` text
RELATIONSHIP_PERIOD       -> relationship_start / relationship_end
ACCOUNTING_PERIOD         -> accounting_start / accounting_end
DOCUMENT_FILING_PERIOD    -> document_filing_start / document_filing_end
```

Known-good Desjardins temporal result has an ACTIVE relationship
beginning 2017-12-06 with no relationship end, while its accounting
period is 2021-01-01 through 2021-12-31.

## 27.5 M4.3A --- internal company expansion CLI --- COMPLETE / VALIDATED

`code/helix_company.py` is now a usable internal operator CLI.

Capabilities validated:

``` text
explicit LEI root resolution
Level 1 identity enrichment
Level 2 accounting relationship expansion
direct + ultimate relationship preservation
unique corporate entities by LEI
corporate confidence kept separate from infrastructure confidence
```

Explicit `--lei` now bypasses fuzzy company-name candidate matching and
performs a direct Level 1 lookup. This prevents a known-good LEI from
being rejected merely because the name search did not include it.

Validated on Desjardins and Sony.
...
``` text
legal name: ソニーグループ株式会社
LEI: 529900R5WX9N2OI2N910
jurisdiction: JP
status: ACTIVE
```

Result: **15 unique related legal entities** after deduplication by LEI.
...
SONY MUSIC PUBLISHING (INDIA) PRIVATE LIMITED
...
Entities that have both direct and ultimate assertions are emitted once
with both relationships preserved, for example:

``` text
SONY EUROPE B.V.
Relationships: DIRECT_ACCOUNTING_CHILD, ULTIMATE_ACCOUNTING_CHILD
LEI: 5493003AP2HY7BTV8H64
```

Backward compatibility is retained through the singular `relationship`
field; the complete aggregated representation is in `relationships`.

## 27.6 Known name-resolution gap --- OPEN, NOT CURRENT BLOCKER

Sony demonstrates that Level 1 primary legal names alone are
insufficient for a friendly company-name resolver. The Sony Group root
is stored under its Japanese legal name, so `--company "Sony"` cannot
reliably discover the correct root from primary `legal_name` matching
alone.

Future improvement: index GLEIF alternative-language names, other entity
names, and transliterations in a separate searchable alias/name table,
conceptually:
...
Also improve token/prefix/word-boundary ranking so substring matches
such as names that merely contain the letters "sony" do not crowd out
real Sony entities.

Do not block domain-layer work on this. Explicit LEI resolution is
sufficient for current testing.

## 27.7 NEXT --- M4.3B domain candidate/evidence layer

Next engineering task:

> Build the corporate-entity-to-domain evidence layer, beginning with
> `code/domain_candidates.py` as the orchestration layer and keeping
> individual evidence providers isolated underneath it.

Input should be the unique corporate entity set produced by the operator
path.

Initial candidate record contract should preserve at least:
...
infrastructure_attribution_confidence
evidence source(s)
...
Do not infer domain ownership merely from corporate ownership. Candidate
domains need independent infrastructure/domain evidence.

Only AUTO candidates and operator-approved REVIEW candidates should
become clean registrable-domain input for Subfinder.

Target downstream artifact can be something like:

``` text
data/processed/<company>_domains.txt
```

but the evidence-bearing JSON should remain the auditable source behind
that flat input list.

Likely evidence adapters over time include official company websites,
EDGAR references, RDAP/WHOIS, certificate/TLS evidence,
customer-supplied data, and future licensed providers. Evidence-source
availability should drive confidence; no single provider should become
the architecture.

## 27.8 M&A and historical identity remain required

GLEIF accounting relationships are only one corporate-evidence source.
The long-term operator workflow must still include:
...
former legal names
historical subsidiaries
absorbed / converted entities
brands / legacy organizations
review candidates
```

EDGAR/event extraction and future M&A/provider adapters should feed the
same canonical corporate candidate pipeline rather than becoming a
separate workflow. The parked broad legal-document generalization study
remains off the MVP critical path unless observed operator gaps justify
reopening it.

## 27.9 Resume checklist for the next session

1.  Read Sections 27.1 through 27.9 first.
2.  Confirm the project virtual environment and repository state.
3.  Run a lightweight index preflight before debugging company
    resolution.
4.  Known-good Level 1 count is approximately 3,393,123 for the current
    2026-08-04 snapshot.
5.  Known-good Level 2 supported relationship count is 258,887 for the
    current 2026-08-17 RR snapshot.
6.  Known-good Sony root LEI is `529900R5WX9N2OI2N910`.
7.  Known-good Sony operator result is 15 unique related legal entities.
8.  Do not reopen GLEIF CLI formatting unless a functional issue
    appears.
9.  Start M4.3B with `code/domain_candidates.py` and an evidence-bearing
    domain candidate schema.
10. Preserve corporate confidence separately from infrastructure
    attribution confidence.
11. After domain evidence works, emit approved registrable domains and
    hand them to Subfinder.
12. Use real customer onboarding as validation data: record Helix
    discoveries, manual misses, duplicates, REVIEWs, rejects, and false
    positives.

### Fast known-good commands
...
``` text
RELATED LEGAL ENTITIES (15)
```

If an index appears missing or a known LEI unexpectedly returns NOT
FOUND, inspect SQLite existence, size, tables, and row counts before
changing resolver logic.

------------------------------------------------------------------------

# 28. UPDATED MILESTONE SNAPSHOT

``` text
Foundation / EDGAR / canonical evidence / normalization / resolution
...
GLEIF
  ...
  M4.2B Persistent Level 2 relationship index       COMPLETE
  M4.2B temporal period correction                  COMPLETE / VALIDATED
  M4.3A Internal company expansion CLI              COMPLETE / VALIDATED
  M4.3B Corporate entity -> domain candidates       NEXT

Operator usability / hardening
  GLEIF alternate-name/alias index                  OPEN
  helix_doctor.py index preflight                   OPEN
  aligned Level 1 / Level 2 snapshot dates          CLEANUP

ASM handoff
  evidence-backed domain disposition                NEXT
  approved root-domain export                       UPCOMING
  Subfinder integration                             UPCOMING
  source-yield instrumentation                      UPCOMING

Expansion
  EDGAR/M&A/historical identity combined workflow   AFTER DOMAIN CONTRACT
  second structured provider                        LATER
  broad ~100-company legal generalization study     PARKED
```
